...
function Tile({ label, value, note }: { label: string; value: string; note?: string }) {
  return (
    <div className="punch-corner card-stack-shadow bg-card-stock p-3 text-card-stock-foreground">
      <p className="font-mono text-[0.65rem] tracking-wide text-card-stock-muted uppercase">{label}</p>
...
export default function CostAnalyticsPanel({ projectId }: { projectId: string }) {
  ...
  if (!cost) return <SkeletonRows rows={4} />
  ...
  if (totals.run_count === 0) {
    return (
      <EmptyState
        title="No runs yet"
        description="Cost data appears after your first run. Every LLM call records its token counts and USD cost; this panel aggregates them by model and by day, and estimates what the response cache has saved."
      />
    )
  }

  const maxDaily = Math.max(1e-9, ...daily.map((d) => d.cost_usd))
  const totalModelCost = Math.max(
    1e-9,
    by_model.reduce((s, m) => s + m.cost_usd, 0)
  )
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <Tile label="Cache hits" value={`${totals.cached_run_count} of ${totals.run_count}`} />
        <Tile
      ...
      <PanelSection title="Daily cost · 30 days">
        <div className="punch-corner-lg border border-border bg-card p-4">
            ...
            <tbody>
              {by_model.map((m) => (
                <tr key={m.model} className="border-t border-border">
                  ...
                  <td className="text-right">
                    {((m.cost_usd / totalModelCost) * 100).toFixed(0)}%
                  </td>
                </tr>
              ))}
            </tbody>
