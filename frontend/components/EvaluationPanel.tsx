...
export default function EvaluationPanel({ projectId }: { projectId: string }) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setDatasets((prev) => [
        { id: created.id, name: created.name, item_count: created.item_count, latest_run: null, created_at: created.created_at },
        ...prev,
      ])
      ...
      <div className="flex shrink-0 flex-col gap-2 sm:w-64">
        ...
        {creating && (
          <div className="punch-corner flex flex-col gap-2 border border-border bg-card p-3">
            <label htmlFor="ds-name" className="text-xs tracking-wide text-muted-foreground uppercase">
              Dataset name
            </label>
            ...
            {rows.map((row, i) => (
              <div key={i} className="flex flex-col gap-1">
                <Input
                  ...
                  value={row.input}
                  onChange={(e) =>
                    setRows((prev) => prev.map((r, j) => (j === i ? { ...r, input: e.target.value } : r)))
                  }
                />
                <Input
                  ...
                  value={row.expected}
                  onChange={(e) =>
                    setRows((prev) => prev.map((r, j) => (j === i ? { ...r, expected: e.target.value } : r)))
                  }
                />
              </div>
            ))}
            ...
            <li key={d.id}>
              <button
                ...
                className={`w-full border border-border p-2 text-left font-mono text-xs ${
                  d.id === selectedId ? 'border-primary font-bold' : 'hover:bg-muted'
                }`}
              >
                {d.name} · {d.item_count} items
                <br />
                {d.latest_run
                  ? `acc ${pct(d.latest_run.accuracy)} · halluc ${pct(d.latest_run.hallucination_rate)}`
                  : 'not run'}
              </button>
            </li>
          ))}
            ...
            <div className="mb-2 flex items-center gap-3">
              ...
              {lastRun && (
                <span className="font-mono text-xs">
                  accuracy {pct(lastRun.accuracy)} · hallucination {pct(lastRun.hallucination_rate)} · mean{' '}
                  {lastRun.mean_score.toFixed(2)}
                </span>
              )}
            </div>
            <ul className="flex flex-col gap-1">
          ...
          {runs.length > 0 && (
            <div className="punch-corner-lg card-stack-shadow border border-border bg-card p-4">
              ...
              <ul className="flex flex-col gap-1 font-mono text-xs">
                {runs.map((r) => (
                  <li key={r.id}>
                    acc {pct(r.accuracy)} · halluc {pct(r.hallucination_rate)} · mean {r.mean_score.toFixed(2)} ·{' '}
